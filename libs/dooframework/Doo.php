...
class Doo{
	...
	public static function error(){
		if(self::$_error===NULL){
			self::loadCore('exception/DooError');
			self::$_error = new DooError();
		}
		return self::$_error;
	}
	...
	/**
	 * @return DooCrashLog logging tool for logging, tracing and profiling, singleton, auto create if the singleton has not been created yet.
	 */
	public static function crashlogger(){
		if(self::$_crashlogger===NULL){
			self::loadCore('logging/DooCrashLog');
			self::$_crashlogger = new DooCrashLog(self::conf()->DEBUG_ENABLED);
		}
		return self::$_crashlogger;
	}
	...
	public static function cache($cacheType='file') {
		if($cacheType=='redis'){
			...
			self::loadCore('cache/DooRedisCache');
			self::$_cache['redis'] = new DooRedisCache(Doo::conf()->REDIS);
			return self::$_cache['redis'];
		}
	...
	public static function loadClass($class_name, $createObj=FALSE){
		//modify by dxf 2014-09-28 10:27:18 在module模式下，优先在当前mvc对应目录下加载，找不到则到根mvc对应目录加载
		$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . "class/";
		if(isset(Doo::conf()->PROTECTED_FOLDER_ORI)===true){
			if(file_exists($path . "$class_name.php")===false){
				$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER_ORI . "class/";
				if(file_exists($path . "$class_name.php")===false){
					$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . "class/";
				}
			}
		...
		return self::load($class_name, $path, $createObj);
	}

	/**
	 * Imports the definition of User defined rule. Class file is located at <b>SITE_PATH/protected/rule/</b>
	 * @param string|array $class_name Name(s) of the class to be imported
	 * @param bool $createObj Determined whether to create object(s) of the class
	 * @return mixed returns NULL by default. If $createObj is TRUE, it creates and return the Object(s) of the class name passed in.
	 */
	public static function loadRules($rules){
		//modify by dxf 2014-10-14 17:55:52 在module模式下，优先在当前mvc对应目录下加载，找不到则到根mvc对应目录加载
		$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . "rules/";
		if(isset(Doo::conf()->PROTECTED_FOLDER_ORI)===true){
			if(file_exists($path . "$rules.php")===false){
				$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER_ORI . "rules/";
				if(file_exists($path . "$rules.php")===false){
					$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . "rules/";
				}
			}
		}
		return include($path . "$rules.php");
	}

	/**
	 * General.20140929 在公共类中调用组件模块类文件
	 * Imports the definition of User defined class(es). Class file is located at <b>SITE_PATH/protected/class/</b>
	 * @param string $module_name 模块路径 'app/news/'
	 * @param string|array $class_name 类文件名 如:NewsClass
	 * @param bool $createObj Determined whether to create object(s) of the class
	 * @return mixed returns NULL by default. If $createObj is TRUE, it creates and return the Object(s) of the class name passed in.
	 */
	public static function loadModuleClass($module_name, $class_name, $createObj = FALSE) {
		$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . "class/";
		$uri = 'module/' . $module_name . '/class/';
		if (strpos($uri, '//') !== false) {
			$uri = str_replace('//', '/', $uri);
		}
		if (isset(Doo::conf()->PROTECTED_FOLDER_ORI) === true) {
			$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER_ORI . $uri;
		} else {
			$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . $uri;
		}
		return self::load($class_name, $path, $createObj);
	}
	 ...
	 * @param bool $isStatic
	 * @internal param array|string $moduleUri URI or Controller name of the module
	 * @internal param string $action Action to be called
	 * @return string Output of the module
	 */
	public static function getModuleClass($moduleName, $className, $function=null,
										  $moduleVersion='1.0.0.0', $prm_base=null, $params=null, $isStatic=true){
		// if (is_array($params)) {
...
//		return $result;
	...
	private static function loadFunctionEx($path, $className, $function, $isStatic, $moduleVersion, $prm_base, $params){
		$pure_class_name = basename($className);
		class_exists($pure_class_name, false)===True || require_once($path . "$className.php");
		if ($isStatic){
			if (method_exists($pure_class_name, $function)) {		//增加方法是否存在的判断  hky20170426
				return $pure_class_name::{$function}($moduleVersion, $prm_base, $params);
			}
		}else{
			$class = new $pure_class_name(null);
			if (method_exists($class, $function)) {	//增加方法是否存在的判断  hky20170426
				return $class->{$function}($moduleVersion, $prm_base, $params);
			}
		}
		$ret['ret'] = RetClass::ERROR;
		$ret['attr']['content'] = 'undefined method';
		return $ret;
	}
	...
	public static function loadModel($class_name, $createObj=FALSE){
		...
		if(isset(Doo::conf()->PROTECTED_FOLDER_ORI)===true){
			if(file_exists($path . "$class_name.php")===false){
				$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER_ORI . "model/";
				if(file_exists($path . "$class_name.php")===false){
					$path = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . "model/";
				}
			}
	...
	public static function loadCore($class_name){
		...
		$pure_class_name = basename($class_name);
		class_exists($pure_class_name, false)===True || require_once(self::conf()->BASE_PATH ."$class_name.php");
	}
