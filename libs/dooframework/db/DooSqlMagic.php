...
class DooSqlMagic {
    ...
    protected $multi_sql;
    protected $multi_sql_param = array();
    protected $multi_sql_count = 0;
    ...
    public function setDb($dbconfig, $default_name){
        ...
        $this->dbconfig_org = $this->dbconfig = $dbconfig[$default_name];
        Doo::conf()->DB_TYPE = $this->dbconfig[4];
	...
	//#modify by huanghailin 2013-1-25
    public function getDbConfigList(){
        return $this->dbconfig_list;
    }
    ...
    public function connect(){
        ...
        try{
            ...
            elseif ($this->dbconfig[4]=='pgsql'){
                //修改增加端口 2012-5-21
                $hosts=explode(':', $this->dbconfig[0]);
                if(count($hosts)==2){
                    $port=$hosts[1];
                }else{
					$port=3306;
                ...
                //dxf 2015-01-12 16:30:28 新增MySQL server has gone away判断
                // echo "{$this->dbconfig[4]}:host={$host};dbname={$this->dbconfig[1]};port={$port}";exit();
                // error_reporting(0);
                $this->pdo = new PDO("{$this->dbconfig[4]}:host={$host};dbname={$this->dbconfig[1]};port={$port}", $this->dbconfig[2], $this->dbconfig[3],array(PDO::ATTR_PERSISTENT => $this->dbconfig[5]));
                $arr=error_get_last();//获取刚发生的错误信息，并返回数组，无错返回null.
                if(isset($arr) )  //不为null,则表示出错了
                {
                    LogClass::log_err1('db_connect_error',$arr,__FILE__,__LINE__);
                    $this->pdo = new PDO("{$this->dbconfig[4]}:host={$host};dbname={$this->dbconfig[1]};port={$port}", $this->dbconfig[2], $this->dbconfig[3],array(PDO::ATTR_PERSISTENT => $this->dbconfig[5]));
                }
    ...
    public function reconnect($db_config_name=Null){
        ...
        if(isset($db_config_name)){
            ...
            $dbconfig = $this->dbconfig = $this->dbconfig_org;
        }
        Doo::conf()->DB_TYPE = $this->dbconfig[4];
        if (Doo::conf()->DB_TYPE == 'mysql') {
        	Doo::conf()->DB_TAG = '`';
        ...
        try{
            if ($dbconfig[4]=='sqlite')
                ...
                $host=$hosts[0];
                // error_reporting(0);
                $this->pdo = new PDO("{$dbconfig[4]}:host={$host};dbname={$dbconfig[1]};port={$port}", $dbconfig[2], $dbconfig[3],array(PDO::ATTR_PERSISTENT => $dbconfig[5]));
                $arr=error_get_last();//获取刚发生的错误信息，并返回数组，无错返回null.
                if(isset($arr) )  //不为null,则表示出错了
                {
                    LogClass::log_warn($arr,__FILE__,__LINE__,EnumClass::LOG_TYPE_VAR_DUMP);
                    $this->pdo = new PDO("{$dbconfig[4]}:host={$host};dbname={$dbconfig[1]};port={$port}", $dbconfig[2], $dbconfig[3],array(PDO::ATTR_PERSISTENT => $dbconfig[5]));
                }
    ...
    /**
     * Initiates a multi sql.
     */
    public function multiBegin() {
    	$this->multi = true;
    }
     ...
     * exec multi sql.
     */
    public function multiAdd($query,$param) {
    	if ($this->multi === true) {
    		if ($this->multi_sql_count > 200) {
    			$this->multiExec();
    			$this->multi = true;
    		}
        	$this->multi_sql .= $query;
        	$this->multi_sql_param = array_merge($this->multi_sql_param,$param);
        	$this->multi_sql_count += 1;
    	}
    }
    ...
    public function multiExec() {
    	if ($this->multi === true) {
    		// var_dump($this->multi_sql);
    		// var_dump($this->multi_sql_param);
    		// exit();
	        $this->query($this->multi_sql, $this->multi_sql_param);
	        $this->multi_sql = '';
	        $this->multi_sql_param = array();
	        $this->multi_sql_count = 0;
    ...
    /**
     * Execute a sql to the connected database
    ...
    public function query($query, $param=null){
		...
		if(!empty($this->stmt)){
			//由于pdo在Windows的bug，导致一次运行多条SQL后，不能继续做其它数据库操作
			//把$stmt改为$this->stmt
    ...
    */
    public function fetchAll($query, $param = null, $fetchMode = null, $fetchModeOptionalParam1=null, $fetchModeOptionalParam2=array()) {
        $this->stmt = $this->query($query, $param);
		if($fetchMode!==null) {
			switch($fetchMode) {
				case PDO::FETCH_COLUMN:
					$this->stmt->setFetchMode($fetchMode, ($fetchModeOptionalParam1 !== null) ? $fetchModeOptionalParam1 : 0);
					break;
				case PDO::FETCH_CLASS:
					$this->stmt->setFetchMode($fetchMode, $fetchModeOptionalParam1, $fetchModeOptionalParam2);
					break;
				case PDO::FETCH_INTO:
					$this->stmt->setFetchMode($fetchMode, $fetchModeOptionalParam1);
					break;
				default:
     ...
     * @return array
     */
    public function show_sql_clear(){
    	$sql_list = $this->sql_list;
        $this->sql_list = array();
        return $sql_list;
    }
    ...
    public function find($model, $opt=null){
        ...
        //limit
        if(isset($opt['limit'])){
        	$sqladd['limit'] = $this->sql_limit_filter($opt['limit']);
        }else{
            $opt['limit']='';
            $sqladd['limit']='';
        }
		...
		//jeffli 20101013 增加排序类型 ORDER BY xxx ASC, xxx ASC
        if(isset($opt['asc_1']) && isset($opt['asc_2']) && $opt['asc_1']!='' && $opt['asc_2']!=''){
            $optkeys = array_keys($opt);
            $sqladd['order']= 'ORDER BY '. $opt['asc_1'] .' ASC, '. $opt['asc_2'] . ' ASC';
        }
        //jeffli 20101226 增加排序类型 ORDER BY xxx DESC, xxx DESC
        if(isset($opt['desc_1']) && isset($opt['desc_2']) && $opt['desc_1']!='' && $opt['desc_2']!=''){
            $optkeys = array_keys($opt);
            $sqladd['order']= 'ORDER BY '. $opt['desc_1'] .' DESC, '. $opt['desc_2'] . ' DESC';
        }
                ...
                if($mtype=='has_many'){
                    ...
                    if($opt['limit']!=''){
                        ...
                        if($opt['limit']==1 || $opt['limit']=='first'){
                            ...
                            // $limitstr = $opt['limit'];
                            $limitstr = $this->sql_limit_filter($opt['limit']);
                        }
                        ...
                        if(isset($opt['param']) && isset($where_values) && !empty($whrLimit)){
                            ...
                            $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} WHERE $whrLimit $orderLimit {$limitstr}", $varsLimit);
                        }else if(isset($opt['param']) && !empty($opt['param']) && !empty($whrLimit)){
                            $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} WHERE $whrLimit $orderLimit {$limitstr}", $opt['param']);
                        }else if(isset($where_values) && !empty($where_values) && !empty($whrLimit)){
                            $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} WHERE $whrLimit $orderLimit {$limitstr}", $where_values);
                        }else{
                            if($orderLimit=='ORDER BY  ') $orderLimit = '';
                            $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} $orderLimit {$limitstr}");
                        }
...
//														echo "<h3>$k2 = $v2</h3>";
    ...
    public function insert($model){
        ...
        foreach($obj as $o=>$v){
            if(isset($v) && in_array($o, $model->_fields)){
                ...
                //     $fieldstr .= $o .',';
                // }
                $fieldstr .= Doo::conf()->DB_TAG . $o . Doo::conf()->DB_TAG .',';
            }
        }
        ...
        //#modify by james.ou 2014-6-26 #增加my sql 没有使用auto increment，而使用uuid_short() 是主键时
        //$this->query($sql, $values);
        //return $this->pdo->lastInsertId();

        if ($this->multi === true) {
        	$this->multiAdd($sql,$values);
        	$id = 1;
        }else{
	        $rowCount = $this->query($sql, $values)->rowCount();
	        $id = $this->pdo->lastInsertId();
	        if($id == 0 && $rowCount > 0 && $model->_primarykey){
	            $id = $model->{$model->_primarykey};
	        }
        }
    ...
    public function update($model, $opt=NULL){
            ...
            foreach($obj as $o=>$v) {
                if(in_array($o, $opt['field'])) {
                    if (($opt['setnulls'] === true || isset($v)) && in_array($o, $model->_fields)) {
                        if (is_object($v)){
							//#modify by dxf 2014-09-26 11:34:45 字段名加上``，避免与关键字冲突
                            //$field_and_value .= "$o=$v,";
							// $field_and_value .= "`$o`=$v,";
							$field_and_value .= Doo::conf()->DB_TAG . $o . Doo::conf()->DB_TAG ."=$v,";
                        } elseif ($v === null) {
							//#modify by dxf 2014-09-26 11:34:45 字段名加上``，避免与关键字冲突
							//$field_and_value .= "$o=null,";
							if (Doo::conf()->DB_TAG == 'pgsql') {
								$field_and_value .= Doo::conf()->DB_TAG . $o . Doo::conf()->DB_TAG ." is null,";
							}else{
								$field_and_value .= Doo::conf()->DB_TAG . $o . Doo::conf()->DB_TAG ."=null,";
							}
						} else {
                            $values[] = $v;
							//#modify by dxf 2014-09-26 11:34:45 字段名加上``，避免与关键字冲突
							//$field_and_value .= $o .'=?,';
                            // $field_and_value .= '`'. $o .'`' .'=?,';
                            $field_and_value .= Doo::conf()->DB_TAG . $o . Doo::conf()->DB_TAG ."=?,";
                        }
                    }
                }
            }
        ...
        if(isset($model->{$obj['_primarykey']})){
            ...
            if(isset($opt['limit'])){
				$limitstr = $this->sql_limit_filter($opt['limit']);
                $sql ="UPDATE {$obj['_table']} SET {$field_and_value} WHERE {$where} {$limitstr};";
            }else{
                $sql ="UPDATE {$obj['_table']} SET {$field_and_value} WHERE {$where};";
            }
        }
    ...
    public function delete($model, $opt=NULL){
        ...
        if (isset($opt['limit'])) {
			$limitstr = $this->sql_limit_filter($opt['limit']);
            $sql ="DELETE FROM {$model->_table} WHERE {$opt['where']} {$limitstr}";
        } else {
        ...
        //conditions WHERE param
        if (isset($opt['param']) && isset($where_values)) {
            $rs = $this->query($sql, array_merge( $opt['param'], $where_values))->rowCount();
        } else if (isset($opt['param'])) {
            $rs = $this->query($sql, $opt['param'])->rowCount();
        } else if (isset($where_values)) {
            $rs = $this->query($sql, $where_values)->rowCount();
        } else {
            $rs = $this->query($sql)->rowCount();
        }
        return $rs;
	...
	public function get_tables($db_name) {
		$smt = Doo::db()->query("SELECT TABLE_NAME,TABLE_COMMENT FROM `information_schema`.`TABLES` where TABLE_SCHEMA='$db_name';");
		$tables = $smt->fetchAll();
		return $tables;
	}
	...
	public function get_table_fields($table_name) {
		...
		if (Doo::conf()->DB_TYPE == 'pgsql') {
	        $dbSchema = $dbconf[6];
            $smt = Doo::db()->query(
                "SELECT DISTINCT column_name AS \"Field\", data_type AS \"Type\", is_nullable AS \"Null\",
                column_default AS \"Default\", ordinal_position AS position, character_maximum_length AS char_length,
                character_octet_length AS oct_length FROM information_schema.columns
                WHERE table_name = '$table_name' AND table_schema = '$dbSchema' ORDER BY ordinal_position"
            );
		}else{
			$db_name = $dbconf[1];
			$smt = Doo::db()->query("SELECT `COLUMN_NAME` as `Field`,`COLUMN_DEFAULT`,`DATA_TYPE`,`IS_NULLABLE`,`COLUMN_TYPE`,`COLUMN_KEY` as `Key`,`CHARACTER_MAXIMUM_LENGTH`,`COLUMN_COMMENT` as `Comment` FROM information_schema.`columns` where TABLE_SCHEMA='$db_name' AND table_name='$table_name' order by ORDINAL_POSITION;");
		}
	...
	public function get_table_field_names($table_name) {
		if (Doo::conf()->DB_TYPE == 'pgsql') {
			$dbconf = Doo::db()->getDefaultDbConfig();
	        $dbSchema = $dbconf[6];
            $smt = Doo::db()->query(
                "SELECT DISTINCT column_name AS \"Field\",ordinal_position AS position FROM information_schema.columns
                WHERE table_name = '$table_name' AND table_schema = '$dbSchema' ORDER BY ordinal_position"
            );
		}else{
			$smt = Doo::db()->query("DESC `$table_name`");
		}
	...
	public function sql_limit_filter($limit) {
		if (Doo::conf()->DB_TYPE == 'pgsql') {
			if(strstr($limit,",")){
				$limit_arr = explode(',', $limit);
				$limit1 = (int)$limit_arr[0];
				$limit2 = (int)$limit_arr[1];
	...
	public function sql_page_make($page,$pagesize) {
		if($page == -1){ //不分页，所有数据
			$limit = '';
		}else{
			$page = $page > 0 ? $page : 1;
			if (Doo::conf()->DB_TYPE == 'pgsql') {
				$limit = ' LIMIT ' . ($page * $pagesize) . ' OFFSET ' . $pagesize;
			}else{
